...
import { aggregateMissionSuccess, type TimeRange } from "@/lib/domain/analytics";

interface MissionsSlotProps {
...
export default async function MissionsSlot({ searchParams }: MissionsSlotProps) {
  const params = await searchParams;
  const range = (params.range || "7d") as TimeRange;
  const data = await aggregateMissionSuccess(range);
